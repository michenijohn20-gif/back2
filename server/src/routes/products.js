...
router.get("/", async (req, res) => {
  try {
    ...
    if (catSlugs.length) and.push({ category: { slug: { in: catSlugs } } });
    ...
    const products = rows.map((p) => {
      ...
      for (const v of variants) {
        const pr =
          cond === "GOOD" ? v.priceGood : cond === "FAIR" ? v.priceFair : v.priceExcellent;
        const st =
          cond === "GOOD" ? v.stockGood : cond === "FAIR" ? v.stockFair : v.stockExcellent;
        if (typeof pr === "number") {
